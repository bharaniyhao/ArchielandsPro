...
class OS_Pagination {
	...
	function renderFirst($tag = 'First') {
		$tag ='<span aria-hidden="true" class="glyphicon glyphicon-fast-backward"></span>';
		if ($this->total_rows == 0)
			return FALSE;
		...
		if ($this->page == 1) {
			...
			return ' <a href="javascript:void(0)" onclick="document.getElementById(\'pagehiddenallform\').value=\'1\';document.getElementById(\'pagingform\').submit()" class="" title="First Page">' . $tag . '</a>';
		}
	}
	...
	function renderLast($tag = 'Last') {
		$tag ='<span aria-hidden="true" class="glyphicon glyphicon-fast-forward"></span>';
		if ($this->total_rows == 0)
			return FALSE;
		...
		if ($this->page == $this->max_pages) {
			...
			return '<a href="javascript:void(0)" onclick="document.getElementById(\'pagehiddenallform\').value='.($this->max_pages).';document.getElementById(\'pagingform\').submit()" class="" title="Last Page">' . $tag . '</a>';
		}
	}
	...
	function renderNext($tag = '&gt;&gt;',$tagon = '&lt;&lt;') {
		$tagon = '<span aria-hidden="true" class="glyphicon glyphicon-arrow-right"></span>';
		if ($this->total_rows == 0)
			return $tag;
		
		if ($this->page < $this->max_pages) {
			return (($this->page < ($this->max_pages - $this->links_per_page))?'':'').'<a href="javascript:void(0)" onclick="document.getElementById(\'pagehiddenallform\').value='.($this->page + 1).';document.getElementById(\'pagingform\').submit()" class="" title="Next Page">' . $tagon . '</a>';
		} else {
			return $tag;
		}
	}
	...
	function renderPrev($tag = '&lt;&lt;',$tagon = '&lt;&lt;') {
		...
		$tagon ='<span aria-hidden="true" class="glyphicon glyphicon-arrow-left"></span>';
		if ($this->total_rows == 0)
			return $tag;
		
		if ($this->page > 1) {
			return '<a href="javascript:void(0)" onclick="document.getElementById(\'pagehiddenallform\').value='.($this->page - 1).';document.getElementById(\'pagingform\').submit()" class="" title="Previous Page">' . $tagon . '</a>'.(($this->page > $this->links_per_page)?'':'');
		} else {
			return " $tag";
		}
	}
	...
	function renderFullNav() {
        ...
        function limitList($limit,$formname){
            echo '<select name="limit" onchange="document.'.$formname.'.submit()" class="pagesize">
	...
	function buildForm(){
		...
		$exp = array("keys","view","selproduct","subaction","target","syscheckbox","checkmandate","inputdate","option","viewpage","inputyear","inputmonth","basregid","basdisid","basdepid","basminid","fdsearch","inputusername","inputuserpassword","inputsurname","inputothername","prollid","phoneno","uregion","inputcategory","inputpriority","inputopexception","inputpassword","inputstatus","action_keysup","cirdisid","agencyid","cirunitid","ajaxfilter","frmname","month","year");
		$str = '<form method="post" id="pagingform" name="pagingform" action="" autocomplete="off">
