<?php
	...
	class Login{
		...
		private function signin(){
			...
			$query = "SELECT USR_USERID,USR_STATUS,USR_OTHERNAME,USR_SURNAME,USR_BRCHID,USR_CODE FROM phys_users WHERE USR_STATUS='1' AND USR_USERNAME=".$this->connect->Param('a')." AND USR_PASSWORD=".$this->connect->Param('b')." ";
			// echo $passwrd;die();
			...
			if($stmt){		
				...
				header('Location: ' . $this->redirect);	
					...
					$ufullname ='';
					$type ='003';
					$query = "INSERT INTO eventlog(EVL_EVTCODE,EVL_MOD_MEMID,EVL_MON_NAME,EVL_ACTIVITIES,EVL_IP,EVL_SESSION_ID,EVL_BROWSER) VALUES(".$this->connect->Param('a').",".$this->connect->Param('b').",".$this->connect->Param('c').",".$this->connect->Param('d').",".$this->connect->Param('e').",".$this->connect->Param('f').",".$this->connect->Param('g').")";
	...
	private function setLog($act){
		...
		$query = "INSERT INTO eventlog(EVL_EVTCODE,EVL_MOD_MEMID,EVL_MON_NAME,EVL_ACTIVITIES,EVL_IP,EVL_SESSION_ID,EVL_BROWSER,EVL_RAW) VALUES(".$this->connect->Param('a').",".$this->connect->Param('b').",".$this->connect->Param('c').",".$this->connect->Param('d').",".$this->connect->Param('e').",".$this->connect->Param('f').",".$this->connect->Param('g').",".$this->connect->Param('g').")";
		if($act == "1"){
		$type ='001';
