...
if (empty($fdsearch)) {
	...
	$query = "SELECT * FROM progress  ORDER BY AssessmentDate ASC";
	$input = array();
} else {

	$query = "SELECT * FROM progress AND ( FirstName LIKE " . $sql->Param('a') . " OR AssessmentDate LIKE " . $sql->Param('b')  . " ) ORDER BY AssessmentData ASC";
