...
// Get all books
if (empty($fdsearch)) {
    $query = "SELECT * FROM BookingRecord WHERE BK_STATUS IN ('1','2','3','4') AND BK_PAYMENT_STATUS IN (1, 2) ORDER BY CODE ASC";
    $input = array();
} else {
    $query = "SELECT * FROM BookingRecord WHERE BK_STATUS IN ('1','2','3','4') AND BK_PAYMENT_STATUS IN (1, 2) AND (BK_LTCODE LIKE " . $sql->Param('a') . " OR BK_TITLE LIKE " . $sql->Param('b') . ") ORDER BY CODE ASC";
