...
switch($viewpage){
    ...
    if ($random_token != $duplicatekeeper) {
		...
		$sql->Execute(
			$sql->Prepare("INSERT INTO treatmentList(PatientID, FirstName, OtherName, LastName, DateOfBirth, Email, PhoneNumber, currentcondition, address, symptoms,Occupation,) VALUE(" . $sql->Param('a') . "," . $sql->Param('b') . "," . $sql->Param('c') . "," . $sql->Param('d') . "," . $sql->Param('e') . "," . $sql->Param('f') . "," . $sql->Param('g') . "," . $sql->Param('h') . "," . $sql->Param('i') . "," . $sql->Param('i') . ") "),
