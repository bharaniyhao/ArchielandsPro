...
switch($viewpage){
	case "editmember":
     ...
     if(!empty($keys)){ 
		 $stmt = $sql->Execute($sql->Prepare("SELECT USR_USERNAME,USR_OTHERNAME,USR_SURNAME,USR_PHONENO,USR_EMERGENCYPHONE,USR_EMAIL,USR_BRCHID,USR_LEVEL,USR_STATUS FROM phys_users WHERE USR_CODE = ".$sql->Param('a')." "),array($keys));
	...
	case "save":
					...
					if(is_array($_POST['syscheckbox'])){
						...
						$sql->Execute("INSERT INTO daa_menusubusers(MENUCT_USRUSERID,MENUCT_MENUDETCODE,MENUCT_ADDEDBY) VALUES(".$sql->Param('a').",".$sql->Param('b').",".$sql->Param('c').")",array($keys,$value,$actorid));
						  ...
						  //Build the permission table for the users
							  $sql->Execute("DELETE FROM phys_userspermission WHERE MENUCT_USRUSERID = ".$sql->Param('a')." ",array($keys));
				  
							  if(isset($_POST['syspermission'])){
							  if(is_array($_POST['syspermission']) ){
								  foreach($_POST['syspermission'] as $value){
									  $sql->Execute("INSERT INTO phys_userspermission(MENUCT_USRUSERID,MENUCT_MENUDETCODE,MENUCT_ADDEDBY,MENUCT_STATUS) VALUES(".$sql->Param('a').",".$sql->Param('b').",".$sql->Param('c').",".$sql->Param('d').")",array($keys,$value,$actorid,'1'));
		...
		if($uppercase && $number && $specialChars) {
		  ...
		  //Build the permission table for the users
			  $sql->Execute("DELETE FROM phys_userspermission WHERE MENUCT_USRUSERID = ".$sql->Param('a')." ",array($usrcode));
  
			  if(isset($_POST['syspermission'])){
			  if(is_array($_POST['syspermission'])){
				  foreach($_POST['syspermission'] as $value){
					  $sql->Execute("INSERT INTO phys_userspermission(MENUCT_USRUSERID,MENUCT_MENUDETCODE,MENUCT_ADDEDBY,MENUCT_STATUS) VALUES(".$sql->Param('a').",".$sql->Param('b').",".$sql->Param('c').",".$sql->Param('d').")",array($usrcode,$value,$actorid,'1'));
...
if (empty($fdsearch)) {
	...
	$query = "SELECT * FROM phys_users WHERE  USR_STATUS  IN ('1','0') ORDER BY USR_USERNAME ASC";
	$input = array();
} else {

	$query = "SELECT * FROM phys_users WHERE  USR_STATUS  IN ('1','0') AND ( USR_USERNAME LIKE " . $sql->Param('a') . " OR USR_SURNAME LIKE " . $sql->Param('b')  . " ) ORDER BY USR_SURNAME ASC";
...
$lenght = 10;
$paging = new OS_Pagination($sql, $query, $limit, $lenght,$input, 'action=index&pg=2e5d8aa3dfa8ef34ca5131d20f9dad51&option=573cd763a6914f2f3b4a2c9ea5fd83d5');
//Get All Branches
