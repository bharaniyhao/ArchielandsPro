...
if (empty($fdsearch)) {
	...
	$query = "SELECT * FROM monitoring WHERE moni_status IN ('1','2') ORDER BY moni_status ASC";
	$input = array();
} else {

	$query = "SELECT * FROM monitoring WHERE moni_status IN ('1','2') AND ( moni_status LIKE " . $sql->Param('a') . " OR moni_status LIKE " . $sql->Param('b')  . " ) ORDER BY moni_status ASC";
