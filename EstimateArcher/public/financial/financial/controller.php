...
switch($viewpage){
	case "editmember":
     ...
     if(!empty($keys)){ 
		 $stmt = $sql->Execute($sql->Prepare("SELECT USR_USERNAME,USR_OTHERNAME,USR_SURNAME,USR_PHONENO,USR_EMERGENCYPHONE,USR_EMAIL,USR_BRCHID,USR_LEVEL,USR_STATUS FROM Financial WHERE USR_CODE = ".$sql->Param('a')." "),array($keys));
...
if (empty($fdsearch)) {
	...
	$query = "SELECT * FROM Financial WHERE status IN ('1','2') ORDER BY CustomerName ASC";
	$input = array();
} else {

	$query = "SELECT * FROM Financial WHERE status IN ('1','2') AND ( CustomerName LIKE " . $sql->Param('a') . " OR invoiceNumber LIKE " . $sql->Param('b')  . " ) ORDER BY invoiceNumber ASC";
