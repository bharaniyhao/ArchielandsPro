...
class ApiRequest {
	...
	public function send($body = null) {
		...
		if (!strncmp($data, 'HTTP/', 5)) {
			...
			if (isset($tokens[1]))
				$response->setStatus(intval($tokens[1])) -> setResponseStatus(intval($tokens[1])); 
			if (isset($tokens[2]))
		...
		if ($body !== null){
		$res_val = $response->getResponseStatus();
		$tag = json_decode($body, true);
		$tag["ResponseStatus"] =$res_val;
