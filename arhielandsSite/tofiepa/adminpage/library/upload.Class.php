 ...
 * Author: Ake 
 */
class uploadClass extends engineClass{
 ...
 public function docAttached($file,$neu_name="-1",$size="-1"){
...
public function DuesAttached($file,$neu_name="-1",$size="-1"){
	$format_types = array('application/vnd.ms-excel','text/plain','text/csv','text/tsv');

	$uploadit = $uploadpass_size = $uploadpass_type = true;
	$target_dir = SHOST_REPAYMENT;
	$neu_name = ($neu_name == "-1")?basename($_FILES[$file]["name"]):$neu_name;
	$codify = uniqid("RP");
	$neu_name = $codify.'_'.$neu_name;
	$target_file = $target_dir.DS.$neu_name;
	//check file exits
	if (!file_exists($target_file)) {
		$uploadit = true;
...
$neu_name = str_replace(' ','',$rand_numb.$file['name']);
$_name_ = $file['name'];
$_type_ = $file['type'];
$_tmp_name_ = $file['tmp_name'];
$_size_ = $file['size'] / 1024;
if(in_array($_type_,$ext)){
//echo SHOST_LOANFORM.$neu_name;
if(@move_uploaded_file($_tmp_name_,SHOST_LOANFORM.$neu_name))
{
	return $neu_name;
